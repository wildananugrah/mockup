...
import { AppError, INVALID_PASSWORD_CODE, INVALID_PASSWORD_MESSAGE, INVALID_USERNAME_CODE, INVALID_USERNAME_MESSAGE, User, } from "../user";
...
describe("User Logic Layer", () => {
    ...
    it("should registering a user", async () => {
        try {
            ...
            const result = await userLogic.register(userData.username, userData.password);
            if (result === undefined)
                fail();
    ...
    it("should logging in a user", async () => {
        try {
            ...
            const userRoleTrxService = new UserRoleTrxServiceMock();
            const userLogic = new User(userService, userRoleTrxService, jwtService);
            const result = await userLogic.login(userData.username, userData.password);
            if (result === undefined)
                fail();
            expect(typeof result.token).toBe("string");
            expect(typeof result.expired).toBe("number");
