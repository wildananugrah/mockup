...
const handleGet = async (req: _IRoute): Promise<Response> => {
  return new Response(
    JSON.stringify({ message: `Service 2 response method: ${req.method}` }),
    {
      status: 200,
      headers: {
...
export async function system2Handler(_param: _IRoute) {
  const { method } = _param;
  ...
  return new Response(
    JSON.stringify({ message: `Service 2 response unknown method` }),
    { status: 400 }
  );
}
