...
const handleGet = async (req: _IRoute): Promise<Response> => {
  return new Response(
    JSON.stringify({ message: `Service 3 response method: ${req.method}` }),
    {
      status: 200,
      headers: {
...
export async function system3Handler(_param: _IRoute) {
  const { method } = _param;
  ...
  return new Response(
    JSON.stringify({ message: `Service 3 response unknown method` }),
    { status: 400 }
  );
}
