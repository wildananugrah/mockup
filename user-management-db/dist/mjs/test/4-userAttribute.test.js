...
beforeAll(async () => {
    ...
    const UserAttribute = new UserAttributeService(await pool.connect());
    await UserAttribute.truncate();
    console.log("Starting truncate User table...");
    const user = new UserService(await pool.connect());
    await user.truncate();
});
afterAll(async () => {
    await pool.end();
...
describe("User Service", () => {
    ...
    it("it should update a user", async () => {
        if (userAttributeId === undefined)
            fail();
        const userAttributeService = new UserAttributeService(await pool.connect());
        const dbUserAttribute = await userAttributeService.update(updatedData, userAttributeId);
        if (dbUserAttribute === undefined)
            fail();
    ...
    it("it should delete a user by id", async () => {
        ...
        const userAttributeService2 = new UserAttributeService(await pool.connect());
